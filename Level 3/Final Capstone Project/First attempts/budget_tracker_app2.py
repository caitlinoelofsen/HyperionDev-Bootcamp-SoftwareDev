...
class Budget_Tracker:
    ...
    # Check if directory exists or create directory
    def connect_db(self):
    ...
    def get_valid_date(prompt):
        while True:
            ...
            try: 
                ...
                print("Invalid date format. Please enter a date in YYY-MM-DD foramt.")
    ...
    def add_new_expense(self):
        ...
        cursor = self.db.cursor()
        cursor.execute("SELECT category_id FROM expense_categories WHERE category_id = ?",
                       (category_id,))
    ...
    def add_expense_category(self):
        ...
        cursor = self.db.cursor()
        try:
            cursor.execute("INSERT OR IGNORE INTO expense_categories (category_name) VALUES (?)",
                       (category_name,))
            self.db.commit()
            if cursor.rowcount == 0:
    ...
    def delete_expense_category(self):
        ...
        cursor = self.db.cursor()
        try:
            cursor.execute("DELETE FROM expense_categories WHERE category_id = ?",
                       (category_id,))
    ...
    def update_income_amount(self):
        income_id = self.get_valid_input("Enter income ID: ")
        new_amount = self.get_valid_input("Enter new amount: ")
        cursor = self.db.cursor()
        try:
            cursor.execute("UDATE incomes SET amount = ? WHERE income_id = ? AND user_id = ?",
                           (new_amount, income_id, self.current_user_id))
            self.db.commit()
            if cursor.rowcount == 0:
                print("No income found with given ID.")
            else: 
                print("Income amount updated successfully.")
        except sqlite3.IntegrityError:
            print("Failed to update income amount.")
